...
int main (int argc, char *argv[]) {
     ...
     double gpsd, gpsd1, gpsdn, gps1, *gpsdv, *gpsd1v, gpsdRest = 0;
     ...
     double *rdt, *rtmp, *x0, alpha, dt, othr, *xRest=NULL,
     ...
     hid_t infile_id;
     herr_t hstat;
     int format_version=1, last_gps_sec=-1, last_gps_nsec=-1, nsamples, last_ichunk=-1;
     float bandwidth;
     double scaling_factor, fpo;
     ...
     fprintf (stderr, "%s: generating time segments from sts data\n", argv[0]);
     ...
     if((infile = fopen(H5FileName,"r+")) != NULL){
          ...
          printf("Opened %s [read only mode][format_version = %d]\n", H5FileName, format_version);
          ...
          if (hstat < 0) {
               printf("Error reading attribute format_version\n"); goto fail;
          } else {
          ...
          if (hstat < 0) {
               printf("Error reading attribute dtype\n"); goto fail;
          } else {
          ...
          if (hstat < 0) {
               printf("Error reading attribute detector\n"); goto fail;
          } else {
          ...
          if (hstat < 0) {
               printf("Error reading attribute fpo\n"); goto fail;
          } else {
               printf("[H5File] fpo = %f\n", fpo);
          }
          ...
          if (hstat < 0) {
               printf("Error reading attribute bandwidth\n"); goto fail;
          } else {
               printf("[H5File] bandwidth = %f\n", bandwidth);
          }

          hstat = H5LTget_attribute_int(infile_id, "/", "nsamples", &nsamples);
          if (hstat < 0) {
               printf("Error reading attribute nsamples\n"); goto fail;
          } else {
     ...
     // 10 * sigma
     othr = 10.*maxasd*sqrt(bandwidth)/efac;

     printf("Computed parameters:\n");
     printf("dt = %f\n", dt);
     printf("N = %d (length of time series)\n", N);
     ...
     //dset = H5Dopen(infile_id, "1", H5P_DEFAULT);
     ...
     double tseg_start, tseg_end, tchunk_start, tchunk_end;
     ...
     double startgps_aligned = startgps + idelta*dt;
     if ( (startgps_aligned - startgps) > 1.e-30) {
          ...
          printf("startgps is aligned with chunk_1 gpstime + m*dt, good! \n");
     }
     ...
     for (iseg=1; iseg<=nseg; ++iseg){
          ...
          while(ichunk<=last_ichunk){
               ...
               tchunk_end = chunk_time + (nsamples-1)*dt;
               if ( (tchunk_end < tseg_start) || (tchunk_start > tseg_end) ) {
                    // chunk is entirely out of the range
                    ichunk++;
                    continue;
               }
               ...
               // chunk possition in segment
               chunk_i0 = tchunk_start < tseg_start ? 0 : (int)round((tchunk_start - tseg_start)/dt);
               chunk_i1 = tchunk_end > tseg_end ? N-1 : (int)round((tchunk_end - tseg_start)/dt);

               if ((tchunk_start >= tseg_start) && (tchunk_end <= tseg_end)) {
                    // chunk is entirely in the range

                    ichunk++;
                    continue;
               } else if (tchunk_start < tseg_start) {
               ...
               for (i=i0; i<=i1; ++i){
                    xtime[i] = xchunk[i-(i0-i)];
               }
               ichunk++;
          }
          for (ichunk=1; ichunk<=last_ichunk; ++ichunk){
              //  search for chunks > tstart < tend
              //  H5LTread_dataset_float(infile_id, "1", xchunk);
